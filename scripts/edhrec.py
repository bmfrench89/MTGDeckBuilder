...
def save_snapshot(commander, coll_index=None, ttl=CACHE_TTL):
    ...
    with open(tmp, "w", encoding="utf-8") as f:   # good committed snapshot
        # sort_keys because these files are COMMITTED and re-generated by a scheduled
        # runner. Without it, keys that tie on value come out in whatever order the
        # upstream response happened to list them, so back-to-back runs produce diffs
        # that move a line and change nothing — four such commits landed on one branch
        # in a single session (2026-08-20), each one a merge conflict waiting to
        # happen for no information at all. A stable order means a snapshot commit
        # appears only when the FIELD actually changed.
