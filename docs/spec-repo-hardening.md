...
- [x] **deck_sections.py silently deletes cards** (`deck_sections.py:41`). Its local `_QTY`
...
- [x] **Snow-Covered basics are invisible to optimize.py** (`optimize.py:46`). `BASICS`
...
- [x] **Delete-deck confirm() never fires** (`webapp/templates/index.html:62`). A raw
...
- [x] **pytest runs a real git sync** (`tests/test_auth.py:72`). `/sync` POSTs with
...
- [x] **cloud-ex-soldier.attrs.csv rows corrupt** (lines 47/50): `Godo, Bandit Warlord`
  and `Sram, Senior Edificer` written with unquoted commas → both parse as garbage. Fix:
  re-quote.
- [x] **Stale buylist Replaces across four decks** (ur-dragon 17/27 rows, cosmic 9/14,
...
- [x] **data/wishlist.md references two deleted decks** — regenerate via `wishlist.py`.
- [x] **handoff.md is layered/stale**: deleted First Avenger still listed with its own
...
- [x] **codemap.md "Availability tiers"** still documents buys entering the 99 badged
  BUY. Update to the owned-only + buylist-Replaces contract.
- [x] **SKILL.md scripts index** omits `goldfish.py` and `deck_sections.py`. Add.
...
- [x] `mtglib.classify()` curated lookup (`mtglib.py:590`) — joined names never match
  curated role lists (Murderous Rider // Swift End classifies as plain creature).
- [x] `deckcore.advise_card()` in-deck set (`deckcore.py:460`) — can recommend a card
  already in the deck under its joined name.
- [x] `deck_conflicts.scan()` (`deck_conflicts.py:52`) — aggregates by raw spelling, so
  the same physical card under two spellings splits into two "covered" entries.
- [x] `optimize` `enriched_by_key` (`optimize.py:223`) — front-face deck line misses its
  own type data; `untyped` over-reports.
- [x] `optimize` buy passes lack the add-side `name_keys` dedup the owned land pass has —
  a DFC buy can hit the buylist twice under two names.

**Webapp / infra**:
- [x] Surface `singleton_violations` from the ⚡ Optimize and Build-Next-save routes
  (`app.py:874,888`) instead of discarding the report.
- [x] `/refresh` wedges the hosted sync loop: it dirties git-tracked wishlist files that
...
- [x] `pa_wsgi.py` deploy notes claim "no env vars, no secret key" — following them
  deploys with auth OFF. Document `MTG_PASSWORD` in the stub.
- [x] `_safe_next` (`app.py:122`) blocks `//host` but not `/\host` (browser-normalized
  open redirect). Normalize backslashes before the check.
- [x] `spellbook.py` cache: unguarded `json.load` (crashes on corrupt cache — degrade
  contract violation) + non-atomic write. Guard read as cache-miss; write tmp+`os.replace`.
- [x] `edhrec._fetch` + `save_snapshot`: non-atomic writes; corrupt-but-fresh cache
...
- [x] `refresh.py:77` IndexError when a child fails with whitespace-only stderr.
- [x] `optimize` stale text: `--no-buys`/`--buy-threshold` help still say buys are
  "added"; dead `[BUY]` printer branch for swaps/land_swaps.
- [x] `deckcore.BASIC_LAND_NAMES` missing `snow-covered wastes` (hubs disagree with
  `mtglib.is_basic`); same omission in `deck_conflicts.BASICS`.
- [x] `carddb._index_keys` uses the banned naive `split("//")` — use `front_face`.
- [x] `append_buylist` against a buylist lacking a `Replaces` column silently drops every
