...
| deck_stats | curve, colored-pip demand vs sources, role counts, ownership. Source counting splits **spend-restricted lands** (`mana-restricted` @ v2) into `color_sources_restricted` — they no longer inflate the Karsten pool; pre-vocabulary lands count as before and bump `restriction_unknown_lands` (unknown ≠ verified-clean) | mtglib, deckcore |
...
| manabase | hypergeometric consistency: keepable %, source adequacy vs Karsten, risky-on-curve — **plus (2026-08-14) the fetch-target census (`fetch_census`: every fetcher with the number of legal targets its search has in THIS deck; `FETCH_THIN=3`; pre-vocabulary data returns an explicit refusal, never a confident zero) and per-color `restricted` counts** from the report's restricted pool. Ships an `explain` dict beside the numbers (what / why / what healthy looks like) so the CLI, dashboard and app render ONE wording and the caveats sit with the stat they qualify | mtglib |
