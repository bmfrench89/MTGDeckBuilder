...
1. **Card-add advisor with component-scored verdicts** (joliverson's "Card
   Evaluation", flegars' structured Doctor output) — already specced as
   `docs/spec-add-card-advisor.md`; our `assess_card()` does the math today, this
   is wiring + UI.
2. **Dead-weight surfacing** (flegars) — a dashboard/assess line for cards whose
   fit components are all near-zero: "synergizes with nothing here." Cheap: it's a
   filter over per-card scores we already compute. Candidate for the tracker.
3. **Synergy normalized by color-identity baseline** (cardcognition) — upgrade the
   optimizer's gain rule and the staple component from raw inclusion % to
   inclusion-vs-baseline ratio, where EDHREC data provides both numbers. Needs a
   feasibility check on what our cached EDHREC payloads actually contain.

**Consider later:** tunable add/cut thresholds as query/CLI params (joliverson);
"deck targets" overrides on `auto_build` role templates + a collection-coverage
line in its output (CyberBelligerent); price-dip alerts on the buylist (OpenMTG —
conflicts with "no live price feed" locked decision, so estimates-only if ever).
